...
export const handleCommitPush = async (): Promise<void> => {
  ...
  try {
    ...
    if (res.data) {
      ...
      const text = `New code has been committed since the last review of <${pull_request._links.html.href}|*PR ${pull_request.number}*>, please review the updates.`;
      // const text = await createUsersToAtString(distinctPreviousReviewers);
